...
// Canvas redraw
let redraw = function(userArr, collect, smooth) {
  var xCoord, yCoord;
  ...
  ctx2.drawImage(collect_ava, collect.x, collect.y, 20, 25);            // Use centre of image for x,y?  
  userArr.forEach((elm) => {
...
// On receiving player update every 0.1 seconds, update user array and collectible and redraw
socket.on('player update', (data) => {
  ...
  var smoothMove = setInterval(() => {
    if (smooth == 10) {
      ...
      redraw(userArr, collect, smooth);
      smooth++;
    };
  }, 10);       // 100 movements per second/10 per server update
});
